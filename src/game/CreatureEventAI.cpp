...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
        ...
        case ACTION_T_CAST:
        {
            ...
            bool canReach = (target->GetTypeId() == TYPEID_PLAYER) && caster->IsWithinLOSInMap(target) && caster->IsWithinLOS(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ());;
